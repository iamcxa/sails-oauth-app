...
parasails.registerComponent('ajaxForm', {
  ...
  methods: {
    ...
    validate: function(formData, formErrors) {
      for (let fieldName in this.formRules) {
        ...
        for (let ruleName in this.formRules[fieldName]) {
          ...
          } else if (ruleName === 'minLowerString' && _.isNumber(ruleRhs)) {
            // ® Must consist of at least this many characters
            violation = (
              !_.isString(fieldValue) ||
              (fieldValue.match(/[a-z]/g) || []).length < ruleRhs
            );
          } else if (ruleName === 'minUpperString' && _.isNumber(ruleRhs)) {
            // ® Must consist of at least this many characters
            violation = (
              !_.isString(fieldValue) ||
              (fieldValue.match(/[A-Z]/g) || []).length < ruleRhs
            );
          } else if (ruleName === 'minDigit' && _.isNumber(ruleRhs)) {
            // ® Must consist of at least this many characters
            violation = (
              !_.isString(fieldValue) ||
              (fieldValue.match(/[0-9]/g) || []).length < ruleRhs
            );
          } else if (ruleName === 'minSpecialString' && _.isNumber(ruleRhs)) {
            // ® Must consist of at least this many characters
            violation = (
              !_.isString(fieldValue) ||
              (fieldValue.match(/[!@#$%^&*]/g) || []).length < ruleRhs
            );
          ...
          if (violation) {
            ...
            formErrors[fieldName].push(ruleName);
    ...
    _submit: async function () {
      ...
      if (this.handleParsing) {
        ...
        let formErrors = this.validate(formData, {});
